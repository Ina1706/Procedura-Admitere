...
public class UserServiceImpl implements UserService{
    ...
    private SortedMap<String, String> persoane;
    ...
    public UserServiceImpl() {
        ...
        persoane = new TreeMap<String, String>();
    ...
    public Liceu adaugareLiceu(){
        ...
        if(identic == 0)
        {
            ...
            fileWriterService.adaugareLiceu(liceuCand);
            System.out.println("intra in user");
        }
        return liceuCand;
    }
    ...
    public void adaugareCandidat(){
        ...
        nrCandidati++;
        FileWriterService fileWriterService = new FileWriterServiceImpl();
        fileWriterService.adaugareCandidat(new Candidat(a, b, c, d, e, f, g, h, i, j));
        persoane.put(c, "Candidat");
    }
    ...
    public void adaugareProfesor(){
        ...
        nrProfesori++;
        FileWriterService fileWriterService = new FileWriterServiceImpl();
        fileWriterService.adaugareProfesor(new Profesor(a, b, c, d, e, f, g));
        persoane.put(c, "Profesor");
    }
    ...
    public void solicitareContestatie(){
        ...
        List<Contestatie> cont = new ArrayList<Contestatie>();
        for(int i = 0; i < nrExamene; i++)
        {
            ...
            cont.add(new Contestatie(notaVeche, notaNoua, gasesteProfesor(disciplina)));
        ...
        FileWriterService fileWriterService = new FileWriterServiceImpl();
        fileWriterService.adaugareContestatie(listaCandidati[index]);
    }
    ...
    public void vizualizareContestatii(){
        ...
        for(int i = 0; i < nrCandidati; i++)
        {
            ...
            {
                System.out.println("CONTESTATIA " + (nrContestatii) + ":");
                listaCandidati[i].afisare();
                nrContestatii ++;
            }
        }
    ...
    public void incarcareCandidati(){
        ...
        int[] numere = fileReaderService.citireCandidati(listaCandidati, listaCandidatiMate, listaCandidatiInfo, listaCandidatiCti, nrCandidati, nrCandidatiMate, nrCandidatiInfo, nrCandidatiCti);
        nrCandidati = numere[0];
        nrCandidatiCti = numere[3];
        nrCandidatiInfo = numere[2];
        nrCandidatiMate = numere[1];
        for (int i = 0; i < nrCandidati; i++){
            int identic = 0;
            Liceu l = listaCandidati[i].getLiceu();
            for(int ind = 0; ind < nrLicee; ind++)
            {
                if (comparaLicee(listaLicee[ind], l) != 0)
                    identic = 1;
            }
            if(identic == 0)
            {listaLicee[nrLicee] = l; nrLicee++;}
        }
    }

    public void incarcaLicee(){
        FileReaderService fileReaderService = new FileReaderServiceImpl();
        nrLicee = fileReaderService.citireLicee(listaLicee, nrLicee);
        for (int i = 0; i < nrLicee; i ++){
            persoane.put(listaCandidati[i].getCnp(), "Candidat");
        }
    }

    public void incarcaProfesori(){
        FileReaderService fileReaderService = new FileReaderServiceImpl();
        nrProfesori = fileReaderService.citireProfesori(listaProfesori, nrProfesori);
        for (int i = 0; i < nrProfesori; i ++){
            persoane.put(listaProfesori[i].getCnp(), "Profesor");
        }
    }

    public void incarcaContestatii(){
        FileReaderService fileReaderService = new FileReaderServiceImpl();
        fileReaderService.citireContestatii(listaCandidati);
    }

    public void statusPersoana(String cnp){
        System.out.println(persoane.get(cnp));
    }
}
